Detect direct calls to the `__clone()` magic method, which is allowed since PHP 7.0."Doing calls like `$obj->__clone()` is now allowed. This was the only magic method
